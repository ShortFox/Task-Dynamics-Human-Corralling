...
public class HA_Controller_LimitedModel : HA_Controller
{
    ...
    private float _dt;                      // Timestep for integration. Will equate to Time.fixedDeltaTime if done in FixedUpdate
    private float _HARt = 0.0f;             // Position Radius from Center
    private float _HAdRdt = 0.0f;           // Velocity Radius from Center
    private float _HAdRvdt = 0.0f;          // Acceleration Radius from Center
    private float _HAThetat = 0.0f;         // Angle from Center
    private float _HAdThetadt = 0.0f;       // Angle Velocity from Center
    private float _HAdThetavdt = 0.0f;      // Angle Acceleration from Center

    private Vector3 _HAPos;                         //Position of HA in task-space coordinates.
    private Vector3 _targetPos;                     //The position the HA will move towards in task-space coordinates.
    private float _targetRad;                       //The target radius HA will move towards.
    private float _targetTheta;                     //The target theta HA will move towards.
    private Vector3 _polRef = Vector3.zero;         //Reference pole for task-dynamic model.
    #endregion

    #region System Parameters
    private float _b_rad;
    private float _epsilon_rad;
    private float _offset_rad;

    private float _b_theta;
    private float _epsilon_theta;
    #endregion
    #endregion
    ...
    {
        // Note, Reset() is called when a trial starts and ends via Task manager
        UpdateParameters();
    ...
    protected override void ComputeState()
    {
        ...
        _polRef = SimulationTask.Instance.TargetAgentsCOM;
        _polRef.y = 0;
    ...
    {
        _b_rad = _b_theta = SimulationTask.Instance.HA_Dampening;
        _epsilon_rad = _epsilon_theta = SimulationTask.Instance.HA_Stiffness;
        _offset_rad = SimulationTask.Instance.HA_Offset;
    }

    private Vector3 SelectTargetPosition(Vector3 reference)
    {
        ...
        // Get direction vector with reference to reference.
